...
const RentForm = (choosenCommunity) => {
  ...
  const isStartDateValid =
    /^(?!0000)[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$/.test(
      startDate
    );
  const isFinishDateValid =
    /^(?!0000)[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$/.test(
      finishDate
    );
    ...
    <Container>
      ...
      >
        <Typography variant="h6" fontWeight="bold">
          {`Formularz dodawania ogłoszenia wypożyczenia przedmiotu dla osiedla: ${capitalizeFirstLetter(
            authCtx.commName
          )} `}
        </Typography>
      </Box>
